        ...
        for airdrop in airdrops:
            try:
                ...
                promotion = {
                    ...
                    'award_token': token,
                    'total_prize_pool': airdrop.get('totalPrizePool', ''),
                    'participants_count': airdrop.get('applyNum', 0),
    ...
    def _parse_mining_data(self, raw_data: Dict) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            promotion = {
                ...
                'title': 'Trade to Earn',
                'award_token': 'WXT',
                'total_prize_pool': f'Up to {max_rate}% rebate' if max_rate else 'WXT Rewards',
                'start_time': start_time,
                'end_time': end_time,
                'link': 'https://www.weex.com/events/futures-trading/trade-to-earn',
                'status': status,
                'type': 'mining',
                'exchange': 'weex',
    ...
    def _convert_to_promotion(self, item: Dict) -> Optional[Dict]:
        ...
        try:
            ...
            return {
                'promo_id': promo_id,
                'title': title,
                'award_token': item.get('token', ''),
                'total_prize_pool': item.get('totalPrizePool', item.get('reward', '')),
                'start_time': item.get('startTime'),
                'end_time': item.get('endTime'),
                'status': 'ongoing',
                'type': 'activity',
